...
export async function loginUser(
  ...
  try {
    const response = await fetch("http://169.254.21.167:8000/login", {
      method: "POST",
...
export async function getPosition(userId: number) {
  try {
    const response = await fetch(
      `http://169.254.21.167:8000/get-locations/${userId}`
    );
    if (response.ok) {
...
export async function updateDriver(
  ...
  try {
    const response = await fetch("http://169.254.21.167:8000/update-driver/", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        user_id: userId,
